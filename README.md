...
- 정책 뉴스, 가격 추이, 기타 인사이트, 매수 준비도, 매수 기회 알림, 오늘의 한마디 섹션을 매일 생성합니다.
- 가격 추이는 일자별 실거래 `중앙값` 기준으로 산출합니다. (극단값 왜곡 완화)
...
`config/report.yaml`의 `buyer_profile` 값을 수정하면 `매수 준비도`와 `매수 기회 알림` 섹션 계산에 반영됩니다.
